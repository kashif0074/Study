...
export default function HomeScreen() {
  ...
  const { user, setShowAuth, colors } = useAuth(); // ✅ Get colors from context
  ...
  const responsiveWidth = Math.min(width, 500); // Max width for content
  ...
  const handleFabPress = () => {
    if (!requireLogin("Organize notes")) return;
  ...
  const StatCard = memo(({ label, value, color }) => {
    ...
    return (
      <View style={[styles.statCard, { 
        ...
        <Text style={styles.statLabel(isSmallScreen, isTablet)}>{label}</Text>
      </View>
    );
  });
  ...
  const QuickActionButton = memo(({ icon, title, subtitle, color, onPress }) => {
    ...
    return (
      ...
      >
        ...
        >
          <Ionicons name={icon} size={isSmallScreen ? 18 : isTablet ? 26 : 22} color="#fff" />
        </LinearGradient>
        <Text style={[styles.quickActionTitle(isSmallScreen, isTablet), { color: theme.text }]}>{title}</Text>
        <Text style={[styles.quickActionSubtitle(isSmallScreen, isTablet), { color: theme.subText }]}>{subtitle}</Text>
      </TouchableOpacity>
    );
  });
  ...
  const RecentActivityCard = memo(
    ({ type, subject, duration, title, summary, progress, date, onContinue, onShare }) => {
      const typeColors = { PDF: "#F56565", Text: "#6B21A8", Voice: "#6B21A8", Image: "#C4B5FD" };
      const typeBg = { PDF: "#FEE2E2", Text: "#E9D5FF", Voice: "#EDE9FE", Image: "#FAF5FF" };
      const color = typeColors[type] || "#6B21A8";
      const backgroundColor = typeBg[type] || "#F3E8FF";
      ...
      return (
        <View style={[styles.activityCard, { 
          ...
          <View style={styles.activityHeader}>
            ...
            <Text style={[styles.activitySubject(isSmallScreen, isTablet), { color: theme.subText }]}>{subject}</Text>
            <Text style={[styles.activityDuration(isSmallScreen), { color: theme.subText }]}>• {duration}</Text>
            <TouchableOpacity style={styles.ellipsis}>
              <Ionicons name="ellipsis-horizontal" size={isSmallScreen ? 16 : 18} color={theme.subText} />
            </TouchableOpacity>
          </View>
          <Text style={[styles.activityTitle(isSmallScreen, isTablet), { color: theme.text }]}>{title}</Text>
          <Text style={[styles.activitySummary(isSmallScreen, isTablet), { color: theme.subText }]} numberOfLines={2}>
            {summary}
          </Text>
          ...
          <View style={styles.activityFooter}>
            <Text style={[styles.activityDate(isSmallScreen), { color: theme.subText }]}>{date}</Text>
            <View style={styles.activityActions}>
              <TouchableOpacity onPress={onContinue}>
                <Text style={[styles.activityActionText(isSmallScreen), { color: theme.primary }]}>Continue</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={onShare} style={{ marginLeft: 16 }}>
                <Text style={[styles.activityActionText(isSmallScreen), { color: theme.primary }]}>Share</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.bg }]}>
      ...
      >
        {/* Gradient Header */}
        <LinearGradient colors={[colors.primary, colors.primary, "#4C1D95"]} style={styles.header}>
          <View style={styles.greetingHeader}>
            ...
            <View style={styles.headerRight}>
              <TouchableOpacity style={styles.headerIcon}>
                <Ionicons name="notifications-outline" size={isSmallScreen ? 20 : isTablet ? 28 : 24} color="#fff" />
              </TouchableOpacity>
              <TouchableOpacity
          ...
          <View style={styles.statsRow}>
            <StatCard label="Study Streak" value="12 days" color="#A78BFA" />
            <StatCard label="Notes Created" value="47" color="#6B21A8" />
            <StatCard label="Quiz Score" value="94%" color="#C4B5FD" />
            <StatCard label="Study Time" value="28 hrs" color="#E9D5FF" />
          </View>
        </LinearGradient>
        ...
        <View style={styles.section}>
          <View style={styles.recentActivityHeader}>
            ...
            <View style={[styles.recentActivityTabs, { backgroundColor: theme.card }]}>
              ...
              <TouchableOpacity
                style={[styles.activityTab, activeTab === "Favorites" && styles.activityTabActive]}
                onPress={() => setActiveTab("Favorites")}
              >
                <Text
                  style={[
                    styles.activityTabText(isSmallScreen),
                    activeTab === "Favorites" && styles.activityTabActiveText,
                  ]}
                >
                  Favorites
                </Text>
          ...
          {activeTab === "Recent" && (
            <>
              <RecentActivityCard
                ...
                summary="Overview of quantum principles, wave-particle duality, and Heisenberg uncertainty principle."
                progress={85}
                date="Dec 13, 2025"
              ...
              <RecentActivityCard
                ...
                summary="Lecture on compound synthesis, reaction mechanisms, and lab procedures."
                progress={67}
                date="Dec 11, 2025"
                onContinue={handleContinue}
                onShare={() => requireLogin("share note") || console.log("Share")}
          ...
          <TouchableOpacity style={[styles.viewAllBtn, { 
            ...
            <Text style={[styles.viewAllText(isSmallScreen, isTablet), { color: theme.primary }]}>View All Notes</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
      ...
      >
        ...
        <Animated.View style={{ transform: [{ scale: fabScale }] }}>
          <Ionicons name="add" size={isSmallScreen ? 24 : isTablet ? 36 : 32} color="#fff" />
        </Animated.View>
      </TouchableOpacity>
    </SafeAreaView>
  );
